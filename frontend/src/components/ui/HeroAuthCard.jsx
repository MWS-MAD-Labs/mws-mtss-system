...
// Hub is the single sign-in entry point now - this used to also offer an
// email/password form backed by this app's own /auth/login, but keeping two
// auth paths in sync (and users' passwords around at all) wasn't worth it
// once Hub SSO covers everyone. See HeroSection.jsx's onGoogleSignIn.
const HeroAuthCard = memo(({ onGoogleSignIn, isCheckingHub = false }) => (
  <div className="relative rounded-3xl border border-border/40 bg-card/70 backdrop-blur-2xl shadow-2xl overflow-hidden">
    ...
    <div className="relative z-10 p-6 md:p-8 space-y-5">
      ...
      <div className="text-center space-y-1.5">
        ...
        <p className="text-xs text-foreground/55">Access your MTSS dashboard</p>
      </div>

      <button
        onClick={onGoogleSignIn}
        disabled={isCheckingHub}
        className="w-full group/btn relative overflow-hidden rounded-xl bg-gradient-to-r from-primary to-primary/85 hover:from-primary/95 hover:to-primary shadow-lg shadow-primary/20 hover:shadow-xl hover:shadow-primary/30 hover:scale-[1.015] hover:-translate-y-px active:scale-[0.985] transition-all duration-300 disabled:opacity-70 disabled:cursor-wait disabled:hover:scale-100 disabled:hover:translate-y-0"
      >
        <div className="relative z-10 flex items-center justify-center gap-3 px-5 py-3.5">
          {isCheckingHub ? (
            <span className="text-sm font-semibold text-white">Checking your Hub session…</span>
          ) : (
            <>
              ...
              <span className="text-sm font-semibold text-white">Sign in with Hub</span>
              <ArrowRight className="w-4 h-4 text-white/70 group-hover/btn:translate-x-1 transition-all duration-300" />
            </>
          )}
        </div>
      </button>
      ...
      <p className="text-[9px] md:text-[10px] text-center text-foreground/40 leading-relaxed">
        Secure authentication powered by Hub SSO.
